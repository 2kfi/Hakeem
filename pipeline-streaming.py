...
@app.post("/prosess-audio-stream")
async def prosess_audio_stream(file: UploadFile = File(...)):
    request_id = uuid.uuid4().hex
    ...
    async def audio_stream_generator():
        ...
        # Use a pipe to stream data from a synchronous thread to the async loop
        ...
        def _synth_thread():
            try:
                ...
                # Close the write end of the pipe from the thread
                # This will signal EOF to the reader
                pass # The with statement handles closing

        # Run the synchronous synthesis in a background thread
        synth_task = loop.run_in_executor(None, _synth_thread)

        try:
            with os.fdopen(read_fd, 'rb') as read_pipe:
                while True:
                    # Read from the pipe in a non-blocking way within the async context
            ...
            # Ensure the synthesis task is complete and resources are cleaned up
            await synth_task
